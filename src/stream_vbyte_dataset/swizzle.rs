    ...
    LaneCount<N>: SupportedLaneCount,
{
    ...
    unsafe {
        match N {
            ...
            8 => transize(vtbl1_u8, bytes, idxs),
            #[cfg(target_feature = "ssse3")]
            // CHANGE: removed `zeroing_idxs`, as they are assumed to be 0xFF already
            16 => transize(x86::_mm_shuffle_epi8, bytes, idxs),
            #[cfg(target_feature = "simd128")]
            16 => transize(wasm::i8x16_swizzle, bytes, idxs),
            #[cfg(all(
                any(target_arch = "aarch64", target_arch = "arm64ec"),
                target_feature = "neon",
                target_endian = "little"
            ))]
            16 => transize(vqtbl1q_u8, bytes, idxs),
            #[cfg(all(
                target_arch = "arm",
                target_feature = "v7",
                target_feature = "neon",
            ...
            16 => transize(armv7_neon_swizzle_u8x16, bytes, idxs),
            #[cfg(all(target_feature = "avx2", not(target_feature = "avx512vbmi")))]
            32 => transize(avx2_pshufb, bytes, idxs),
...
unsafe fn avx2_pshufb(bytes: Simd<u8, 32>, idxs: Simd<u8, 32>) -> Simd<u8, 32> {
    ...
    unsafe {
        ...
        // CHANGE: indices > 32 are always 0xFF, meaning that out of bounds indices are zeroed already/
