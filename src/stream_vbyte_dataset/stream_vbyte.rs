...
impl<'a> StreamVbyte<'a> {
    ...
    pub unsafe fn from_unchecked(slice: &'a [usize], n_elems: usize) -> Self {
        unsafe {
            ...
            let idx_lens_start = values_end.next_multiple_of(align_of::<u8>());
            ...
            // TODO: this unbounded length is "safe": due to the padding, *after the swizzle*, the only read values are of this posting and they are 0 (which is in bounds of the query vector, even with the prefix sum).
    ...
    pub fn push_posting(vec: &mut Vec<u8>, converted_components: &mut [u16], values: &mut [u8]) {
        ...
        let bitvec: Vec<u8> = converted_components
            .chunks(N)
            .map(|chunk| {
                let mut byte = 0;
        ...
        let padding = N - converted_components.len() % N;
        ...
        unsafe {
            ...
            for &c in converted_components.iter() {
                if c > u8::MAX as u16 {
            ...
            vec.resize(
                (vec.len() + padding * size_of::<u8>()).next_multiple_of(size_of::<Simd<u8, N>>()),
                0,
            );
        }
    }

    pub fn iter_raw(self) -> impl ExactSizeIterator<Item = (Simd<u16, N>, Simd<u8, N>)> {
    ...
    pub fn iter_with_prefix_sum(
        self,
    ) -> impl ExactSizeIterator<Item = (Simd<u16, N>, Simd<u8, N>)> {
        let mut last_component = 0;
        self.iter_raw().map(move |(c, v)| {
