use clap::Parser;
use seismic::{FixedU8Q,InvertedIndex};
use seismic::sparse_dataset::{SparseDataset, SparseDatasetMut, SparseDatasetTrait};
...
fn main() {
    ...
    // Use the uncompressed SparseDataset with u16 components and fixedu8 values
    let inverted_index: InvertedIndex<SparseDataset<u16, FixedU8Q>> =
    ...
    for (q_components, q_values) in queries.dataset_iter().take(n_queries) {
        // Prepare the query once for this dataset implementation (explicit types help inference)
    ...
    println!(
        ...
        elapsed.as_micros() / n_queries as u128
    );
