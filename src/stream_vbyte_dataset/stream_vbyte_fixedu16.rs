...
#[derive(Clone)]
pub struct StreamVbyteFixedu16<'a> {
    values: &'a [Simd<u16, N>],
    bytes_remaining: &'a [u16],
    values_remaining: &'a [u16],
    idx_lens: &'a [u8],
    bytes: &'a [u8],
}

impl<'a> StreamVbyteFixedu16<'a> {
    // We have got:
    ...
    pub unsafe fn from_unchecked(slice: &'a [usize], n_elems: usize) -> Self {
        unsafe {
            ...
            let values_end = n_packs * size_of::<Simd<u16, N>>();
            ...
            let values_remaining_start = bytes_remaining_end.next_multiple_of(align_of::<u16>());
            let values_remaining_end = values_remaining_start + n_remaining * size_of::<u16>();
    ...
    pub fn push_posting(vec: &mut Vec<u8>, converted_components: &mut [u16], values: &mut [u16]) {
        ...
        unsafe {
            ...
            vec.resize(vec.len().next_multiple_of(size_of::<Simd<u16, N>>()), 0);
        }
    }

    fn iter_raw(self) -> impl ExactSizeIterator<Item = (Simd<u16, N>, Simd<u16, N>)> {
    ...
    pub fn iter(self) -> impl Iterator<Item = (u16, f16)> {
        ...
        gen move {
            ...
            for (c, v) in self.iter_raw() {
                ...
                for (c, v) in c_prefixed_previous
                    .to_array()
                    .into_iter()
                    .zip(v.to_array().into_iter().map(f16::from_bits))
                {
                    yield (c, v);
                }
            }
            for (c, v) in bytes_remaining.iter().zip(values_remaining.iter()) {
                last_component += c;
                yield (last_component, f16::from_bits(*v));
            }
        }
    }
    ...
    pub fn dot_product(&self, mut query: &[f32]) -> f32 {
        ...
        for (components, values) in self.clone().iter_raw() {
            ...
            let values = simd_f16_to_f32(values);
        ...
        let remaining_result = self
            ...
            .scan(0, move |acc, (&c, &v)| {
                *acc += c;
                let query_value = f16::from_bits(v).to_f32();
